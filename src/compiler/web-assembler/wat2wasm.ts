...
//         if (typeof console === "undefined") (console as unknown) = {};
//         const printErr = () => {};
//         console.log = print;
...
//     if (typeof (WebAssembly as any).Function === "function") {
//         var typeNames = {
//             "i": "i32",
//             "j": "i64",
//             "f": "f32",
...
//         }
//         return new (WebAssembly as any).Function(type, func)
//     }
...
//                 const _stackAlloc: (size: number) => number = stackAlloc;
//                 ret = _stackAlloc(len);
//                 stringToUTF8(str, ret, len)
//             }
//             return ret
...
//     ret = convertReturnValue(ret);
//     if (stack !== 0) (stackRestore as any)(stack);
//     return ret
// }
...
// console.log('asd', Module["wasmMemory"]);
...
function __ZSt18uncaught_exceptionv() {
    return (__ZSt18uncaught_exceptionv as any).uncaught_exceptions > 0
}
...
function ___cxa_throw(ptr, type, destructor) {
    ...
    exceptionLast = ptr;
    if (!("uncaught_exception" in __ZSt18uncaught_exceptionv)) {
        (__ZSt18uncaught_exceptionv as any).uncaught_exceptions = 1
    } else {
        (__ZSt18uncaught_exceptionv as any).uncaught_exceptions++
    }
    throw ptr
...
async function run(moduleRes: BufferSource) {
    // prepare imports
    const info = {
        'env': asmLibraryArg,
        'wasi_snapshot_preview1': asmLibraryArg
    ...
    // const instantiated = await WebAssembly.instantiateStreaming(moduleRes, info);
    const instantiated = await WebAssembly.instantiate(moduleRes, info);

    const exports: any = instantiated.instance.exports;
    // const exports: any = instantiated.exports;
...
async function fetchRun(wasmModulePath?: string) {
    ...
    // return run(fetchedWasm);
    return run(await fetchedWasm.arrayBuffer());
}

async function readRun(wasmBin?: BufferSource) {
    // const res = new Response(wasmBin, { status: 200 });

    return run(wasmBin);
}

function getCompiledFunc(exports) {
    const {
        free,
        malloc,
        free_memory,
